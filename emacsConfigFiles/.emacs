...
;; add horizontal scroll bar (as opposed to using the arrows)
(custom-set-variables
 '(horizontal-scroll-bar-mode t))

;; gives emacs the same path variables whether it is started from GUI or Terminal 
(when (memq window-system '(mac ns x))
  (load "exec-path-from-shell")
  (exec-path-from-shell-initialize))

;; Set the Environment for a Windows computer
(when (memq window-system '(w32 ns x))
	;; list all files/folders in C:/ with ADMB, R, and Rtools in it -- nil is equivalent to NULL
	(setq ADMBFiles (directory-files "C:/" nil "ADMB"))
	(setq RFiles (directory-files "C:/Program Files/R" nil "R-"))
	(setq RtoolsFiles (directory-files "C:/" nil "Rtools"))

	;; get length of the list of ADMB and R files 
	(setq ADMBFileLength (length ADMBFiles))
	(setq RFileLength (length RFiles))
	(setq RtoolsLength (length RtoolsFiles))

	;; subtract 1 from length (indexes start with 0)
	(setq ADMBFileLength (- ADMBFileLength 1)) 
	(setq RFileLength (- RFileLength 1)) 
	(setq RtoolsLength (- RtoolsLength 1)) 

	;; get the last value in the list (in case there are multiple installation -- the last is the highest version)
	(setq lastADMBFile (nth ADMBFileLength ADMBFiles))
	(setq lastRFile (nth RFileLength RFiles))
	(setq lastRtoolsFile (nth RtoolsLength RtoolsFiles))

	;; Add rtools and ADMB to the Environment PATH
	(setenv "PATH"
	  (concat
		"C:/Program Files/R/" lastRFile "/bin;"
		"C:/" lastRtoolsFile "/usr/bin;"
		"C:/" lastRtoolsFile "/mingw64/bin;"
		"C:/" lastADMBFile "/bin;"
		"C:/" lastADMBFile "/utilities/mingw64/bin;"
		(getenv "PATH")
	  )
	)
	
	;; fix issue with looking for Rterm, which is not needed anymore (this is also a Mac issue...)
	(setq Rexe (concat "C:/Program Files/R/" lastRFile "/bin/R.exe"))
	(message Rexe) 
	(setq inferior-ess-r-program Rexe)
)